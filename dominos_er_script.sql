...
CREATE TABLE IF NOT EXISTS `mydb`.`orders_have_pizzas` (
  ...
  CONSTRAINT `pizza_breads_fk`
    ...
    REFERENCES `mydb`.`pizza_breads` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `pizza_sizes_fk`
    FOREIGN KEY (`pizza_sizes_id`)
    REFERENCES `mydb`.`pizza_sizes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`orders_have_other_products` (
  ...
  CONSTRAINT `other_products_fk`
    ...
    REFERENCES `mydb`.`other_products` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
